...
def placeholder_or_random_sample_text():
    """Gets a random sample chunk of text if possible, otherwise the default placeholder text."""
...
def random_sample_text(num_sentences=3):
    ...
    if not text:
        url = random.choice(app.config['EDITABLE_SAMPLE_TEXT_SCRAPE_URLS'])

        r  = requests.get(url)

        # Thanks to:
        # https://github.com/kennethreitz/requests/issues/1604#issuecomment-24476927
        r.encoding = 'utf-8'
    ...
    return '<p>{0}</p>'.format(' '.join([unidecode(text_model.make_sentence().decode('utf-8').strip()) for i in range(num_sentences)]))
